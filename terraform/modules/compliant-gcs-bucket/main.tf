# main.tf
terraform {
  required_version = ">= 1.9" # cross-variable validation in variables.tf requires 1.9+
  required_providers {
...
resource "google_storage_bucket" "bucket" {
  ...
  retention_policy {
    ...
    # Locking is a one-way door on real GCP infrastructure: once true, it can
    # never be unset or reduced, even by the project owner. Left false for
    # this module; a production variant should lock deliberately in code,
    # not via a variable a consumer could flip.
    is_locked = false
  }
